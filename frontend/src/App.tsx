...
const socket: any = io(server, connectionOptions);
...
const App: React.FC = () => {
  ...
  const videoGrid = useRef<any>(null);

  const addVideoStream = (div: HTMLDivElement, video: HTMLVideoElement, stream: MediaStream) => {
  ...
  const connectToNewUser = (userId: string, name: string, stream: MediaStream) => {
